...
function sanitizeFilename(name) {
    return name.replace(/[^a-z0-9\s]/gi, '_').replace(/_+/g, ' ').trim().replace(/\s+/g, ' ').toLowerCase()
}
...
function displayVideoStats(file) {
    ffmpeg.ffprobe(file, (err, data) => {
        ...
        console.log(`Format: ${data.format.format_name}, Duration: ${data.format.duration}s, Size: ${data.format.size} bytes`)

        data.streams.forEach((stream, index) => {
            if (stream.codec_type === 'video') {
                console.log(`Video Stream ${index + 1}: Codec: ${stream.codec_name}, Resolution: ${stream.width}x${stream.height}`)
            } else if (stream.codec_type === 'audio') {
                console.log(`Audio Stream ${index + 1}: Codec: ${stream.codec_name}, Bitrate: ${stream.bit_rate}, Sample rate: ${stream.sample_rate}`)
            }
        })
    })
}
        ...
        ffmpeg()
            ...
            .on('progress', (progress) => {
                ...
                if (percentage >= 0) {
                    ...
                    // eslint-disable-next-line no-undef
                    process.stdout.write(`Merging progress: ${percentage}% - Elapsed time: ${formattedTime}\r`)
                }
            })
            .on('end', () => {
...
module.exports = { downloadAndMergeVideo, downloadVideo, downloadAudio, processWithFFmpeg, sanitizeFilename }
